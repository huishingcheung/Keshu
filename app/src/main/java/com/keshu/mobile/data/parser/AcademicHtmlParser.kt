...
class AcademicHtmlParser {
    ...
    private fun parseExams(html: String): ParsedAcademicPage {
        ...
        val cards = doc.select(".scenes-cbrt-card-item, .scenes-card, .ksap-table-container tr")
            .ifEmpty {
                doc.select("li, tr").filter { element ->
                    val text = element.text()
                    text.contains("考试时间") || text.contains("考试地点") || text.contains("座位号")
                }
            }
